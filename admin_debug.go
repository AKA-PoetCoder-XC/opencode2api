...
func (a *AdminServer) handleDebugModels(w http.ResponseWriter, _ *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{"models": models, "keys": keys, "metadata": metadata, "catalog": catalog, "last_inference": last})
}
...
func (a *AdminServer) handleDebugInference(w http.ResponseWriter, r *http.Request) {
	...
	trace := &requestMeta{}
	ctx := context.WithValue(withDiagnosticRequest(r.Context()), requestMetaKey{}, trace)
