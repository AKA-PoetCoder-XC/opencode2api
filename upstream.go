...
func (g *Gateway) dumpOutboundBodies(route modelRoute, bodies map[Tier][]byte, ids requestIDs, attemptOffset int) {
...
// live traffic before.
func (g *Gateway) doSelectedKeyUpstream(ctx context.Context, route modelRoute, bodies map[Tier][]byte, ids requestIDs, override debugKeyOverride, attemptOffset int) (*http.Response, error, int) {
	nodes := g.zenNodes
...
func (g *Gateway) recordUpstreamAttempt(ctx context.Context, route modelRoute, ids requestIDs, attempt int, keyID, channel string, anonymous bool, proxy *proxyTransport, resp *http.Response, err error, duration time.Duration) {
	status := upstreamStatus(resp)
